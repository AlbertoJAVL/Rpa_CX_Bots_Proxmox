...
def lecturaOpciones(driver):
    ...
    while buscandoCancelar == True:
        try:
            ...
            if 'Cancelar' in opcionorden or 'Cancelado' in opcionorden:
...
def ordenesServicio(driver, datos, comentarios, motivoCliente):

    open_item_selenium_wait(driver, xpath = ordenes_servicio['OS'])
    extOrdeneServicio(driver,datos)

    # estatusORden = driver.find_element(By.XPATH, ordenes_servicio['estatusOrden'])
    # estatusORden = estatusORden.text

    # if estatusORden not in ['Cencelado', 'Cancelda']:
    #     error = f'Error cuenta {estatusORden}'
    #     print(error)
    #     return False, error
    ...
    estatusORden = driver.find_element(By.XPATH, ordenes_servicio['estado'] + '/input')
    estatusORden = estatusORden.get_attribute("value")
    if estatusORden in ['Cancelado', 'Cancelada']:
        error = f'Error cuenta {estatusORden}'
        print(error)
        return False, error
    ...
    comentario.clear()
    comentario.send_keys(comentarios)
    ...
    motivoCancelacion.send_keys(motivoCliente)
    ...
    sleep(4)
    # driver.find_element(By.XPATH, ordenes_servicio['estadoCancelado']).click()
...
def generacionCasoNegocio(driver, tipoCN, cuenta, motivoCliente):#50485788
    ...
    if 'tipo 1' in tipoCN:
        ...
        comentario = driver.find_element(By.XPATH, caso_negocio['comentario']).click()
        comentario = driver.find_element(By.XPATH, caso_negocio['comentario'] + '/input')
