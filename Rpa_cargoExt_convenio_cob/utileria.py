...
def pantalla_unica_consulta(driver, cuenta):
    ...
    try:
        ...
        # if open_item_selenium_wait(driver, xpath = home['pantalla_unica']['xpath'] ) != True: return False

        sleep(3)
        driver.find_element(By.XPATH, home['pantalla_unica']['xpath']).click()
        sleep(4)

        # driver.find_element(By.XPATH, '/html/body/div[1]/div/div[3]/div/div/div[1]/div[3]/ul/li[2]/a').click()
        ...
        #LUPA DE BUSQUEDA
        lupa = driver.find_element(By.XPATH, "//button[@aria-label='Pantalla Única de Consulta Applet de formulario:Consulta']").click()
        print('<<<<<<<<<<<' + str(lupa) + '>>>>>>>')
        sleep(10)

        #Se posiciona para escirbir en el campo de numero de orden
        element = driver.find_element(By.XPATH, "//input[@aria-label='Numero Cuenta']")
        ...
        #     try:
        #         sleep(10)
        #         driver.find_element(By.XPATH, pantalla_unica['inputCuenta']).click()
        #         numCuenta = driver.find_element(By.XPATH, pantalla_unica['inputCuenta']+'/div/input')
        #         numCuenta = numCuenta.get_attribute("value")
        #         print(numCuenta)

        #         if cuenta in numCuenta:
